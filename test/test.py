...
async def alu_operations_test(dut):
    ...
    carry = int((dut.uo_out.value >> 4) & 1)
    parity = int((dut.uo_out.value >> 5) & 1)
    ...
    await RisingEdge(dut.clk)
    await Timer(10, units='ns')
    result_active = int(dut.uo_out.value & 0xF)
    ...
    await RisingEdge(dut.clk)
    await Timer(10, units='ns')
    result_disabled = int(dut.uo_out.value & 0xF)
    carry_disabled = int((dut.uo_out.value >> 4) & 1)
    parity_disabled = int((dut.uo_out.value >> 5) & 1)
